...
# 用于三元实体关系提取的promp
EXTRACTOR_PROMPT = (
    "你是一位专业的金融领域知识图谱专家。你的任务是精确地从下面的金融研报文本中提取知识三元组。"
    "请严格按照以下要求操作：\n"
    "1. **识别关键实体**: 主要关注以下几类实体：\n"
    "   - **组织/公司**: 如 '国盛证券', '中国人民银行', '美联储'。\n"
    "   - **经济数据/指标**: 如 '新增人民币贷款', '社融规模', 'M2增速', 'CPI'。\n"
    "   - **政策/事件**: 如 '最新关税政策', '降准', 'LPR改革'。\n"
    "   - **日期/时间**: 如 '4月', '二季度'。\n"
    "   - **金融概念**: 如 '实体经济', '流动性'。\n"
    "2. **提取关系**: 提取实体之间有意义的关系，例如：\n"
    "   - **因果关系**: (实体A, **影响**, 实体B), (实体A, **导致**, 实体B)\n"
    "   - **数据表现**: (经济数据, **同比增加**, 具体数值), (经济数据, **低于**, 市场预期)\n"
    "   - **观点/归属**: (分析师/机构, **认为**, 观点), (报告, **发布于**, 日期)\n"
    "   - **构成关系**: (实体A, **是...的核心驱动**, 实体B)\n"
    "3. **输出格式**: 将所有提取的三元组格式化为一个 JSON 列表，每个三元组是一个包含 'subject', 'predicate', 'object' 键的字典。即使只有一个三元组，也要使用列表格式。如果文本中没有可提取的三元组，请返回一个空列表 `[]`。\n\n"
    "--- 示例 ---\n"
    "文本: '事件：2025 年4 月新增人民币贷款 2800 亿，市场预期为 3500 亿。国盛证券认为，这主要是由于实体经济信贷需求季节性回落所致。'\n"
    "输出: \n"
    "[\n"
    "  {{\"subject\": \"4月新增人民币贷款\", \"predicate\": \"是\", \"object\": \"2800亿\"}},\n"
    "  {{\"subject\": \"4月新增人民币贷款\", \"predicate\": \"低于\", \"object\": \"市场预期\"}},\n"
    "  {{\"subject\": \"国盛证券\", \"predicate\": \"认为\", \"object\": \"实体经济信贷需求季节性回落是主要原因\"}},\n"
    "  {{\"subject\": \"实体经济信贷需求季节性回落\", \"predicate\": \"导致\", \"object\": \"4月新增人民币贷款减少\"}}\n"
    "]\n"
    "--- 结束示例 ---\n\n"
    "现在，请从以下文本中提取三元组。\n\n"
    "文本: '{text}'\n"
...
EXTRACTOR_PROMPT_1 = PromptTemplate(EXTRACTOR_PROMPT)
...
""")
CYPHER_PROMPT = PromptTemplate(CYPHER_PROMPT_RAW)
