...
你是一位金融领域知识图谱专家，你的任务是**高度概括**并从给定的金融研报文本中，提取出**最核心的、不超过 {max_triplets_per_chunk} 个**结构化知识三元组。
...
4.  **严格数量限制**: **这是一个硬性要求**。你返回的三元组总数**绝对不能超过 {max_triplets_per_chunk} 个**。如果文本中有很多信息，请只选择最重要的返回。
5.  **输出格式**: 将所有提取的三元组格式化为一个JSON列表。每个三元组是一个包含 "subject", "predicate", "object" 键的字典。如果文本中没有可提取的核心三元组，请返回一个空列表 `[]`。

# 示例文本与输出 (max_triplets_per_chunk=2)
-------
文本:
"事件：2025 年4 月新增人民币贷款 2800 亿，市场预期为 3500 亿，同比少增。国盛证券在最新的报告中分析并认为，这主要是由于实体经济信贷需求季节性回落所致。"
-------
输出:
[
...
FINANCE_VALIDATION_SCHEMA = {
    ...
    # 日期/时间 通常作为关系的客体，但有时也可以是主体（例如 "2023年发布于...")
    "日期": [
        "发布于",
    ],
    "时间": [
...
3.  使用你找到的最匹配的Schema元素，并且保证查询的三元关系是Schema中已经存在且相连的三元关系，构建一个精确的Cypher查询。
...
2.  **实体提取与精确匹配**: 你必须从用户问题中，提取出**最核心、最不可能变化**的名词作为 `CONTAINS` 的查询词。例如，对于“胜利证券这家公司”，你应该提取 `'胜利证券'`。
    优先使用 `CONTAINS`,对于从用户问题中提取出的核心实体，**必须**在 `WHERE` 子句中使用 `n.name CONTAINS '部分实体名'` 的方式进行匹配。
    **绝对禁止在最终的Cypher代码中出现`{query_str}`这个占位符字符串。**
