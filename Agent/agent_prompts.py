...
DISPATCHER_PROMPT = """
你的任务是分析用户的请求，并从下面的任务类型列表中，判断出最合适的任务类型。
...
SUMMARIZER_PROMPT = """
你是一位顶级的金融分析专家。你的任务是根据下面提供的、经过验证的“上下文信息”，为用户的“原始请求”生成一个专业、精准、深入且完全忠于原文的回答。
用户的原始请求
{user_msg}
上下文信息
{context}
...
CRITIQUE_PROMPT = """
你是一位极其严谨和挑剔的首席分析师（质检员）。你的任务是评估下属提交的“草稿回答”，判断其是否达到了可发布的标准。
用户的原始请求
{user_msg}
下属的草稿回答
{draft_answer}
